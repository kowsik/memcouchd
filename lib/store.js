...
var Store = function() {
    ...
    var _find = function(key, id, key_only_match) {
        ...
        do {
            ...
            if (c === 0) {
                if (!key_only_match) {
                    c = Collator.compare2(id, tid, id2, tid2);                    
                }
            }
    ...
    return {
        ...
        each: function(opts, cb) {
            ...
            count = opts.limit || Math.abs(start.index - end.index) + 1;
            if (opts.descending) {
                for (n = start.index; count > 0 && n >= end.index; --n, --count) {
                    node = _nodes[n];
                    if (cb(node.key, node.val, node.id) === false) {
                        break;
                    }
                }
            } else {
