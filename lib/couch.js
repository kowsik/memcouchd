    ...
    return {
        ...
        dbPutDoc: function(name, docid, doc) {
            ...
            return _ifDBExists(name, function(db) {
                return db.docs.insert2(doc._id, function(_doc, insertcb) {
                    ...
                    // If this is a design doc, kick off the view indexer
                    // for all of the docs. Effectively this means that we
                    // need to rebuild all views for this design.
            ...
            return _ifDBExists(name, function(db) {
                return db.docs.erase2(docid, function(_doc, erasecb) {
                    if (_doc) {
                        ...
                        // TODO: If we are doing async view indexing, then
                        // each view needs to be notified that it's getting
                        // deleted so it can stop any active timers.
        ...
        dbView: function(name, design, view, opts, cb) {
            return _ifDBExists(name, function(db) {
                ...
                // TODO: Check opts.group=true and handle reduce here. The
                // simplest way to do this is from within the callback, keep
                // track of the last key. If it's the same as the current key,
                // we simply rereduce the two keys and accumulate. When the
                // key in the callback changes from the 'last', then we emit
                // the last reduced [k,v] pair and start the new one. So
                // really no storage of large reduced values.
                //
                // There are three possible options here
                // - If _view.hasReduce() and opts.group === undefined, then
                //   default to reducing. Except we are going to ignore the keys
                //   and group everything into a single thing.
                // - If opts.group === true, then we need to uniquify the keys
                // - Finally if opts.reduce === false, then this becomes a pure
                //   mapping function.
