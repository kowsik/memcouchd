...
this.create = function(name, view) {
    ...
    return {
        ...
        each: function(opts, cb) {
            ...
            // - Once the store becomes a BTree, we can cache the reduced
            //   values a whole lot better. Right now it's brute forcing
            ...
            if (reduce) {
                ...
                _store.each(opts, function(key, val, id, node) {
                    // First time around, save the last key, val and the collation type
                    if (last_key === undefined) {
                        last_key  = key;
                        last_val  = _reduce([ key ], [ val ]);
                        last_tkey = node.tkey;
                    } else {
                        // If the current key is the same as the last one, then 
                        // incrementally re-reduce the values
                        if (Collate.compare2(key, node.tkey, last_key, last_tkey) === 0) {
                            last_val = _reduce(null, [ last_val, val ], true);
                        } else {
                            // When the key changes, we are on to the next
                            // "group". So emit what we have and start all 
                            // over again.
                ...
                // Flush the last group, if we are in reduce mode
                if (last_key) {
                    cb({ key: last_key, value: last_val });
                }                
            } else {
